...
export default function QuestionnaireTemplatesPage() {
  ...
  const { data, isLoading } = useQuery<{ data: Template[] }>({
    queryKey: ['intake-templates-admin', 'common'],
    queryFn: () => api.get('/intake/templates/admin?section_type=common'),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      editing
        ...
        : api.post('/intake/templates', { ...payload, sectionType: 'common' }),
    onSuccess: () => {
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-white rounded-lg border border-border-light p-6 mb-6">
          <h2 className="font-medium mb-4">{editing ? 'Edit Question' : 'New Common Question'}</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input className="input-field" value={form.options} onChange={(e) => setForm({ ...form, options: e.target.value })} placeholder="Option A, Option B" />
              </div>
      ...
      {!isLoading && templates.length === 0 && (
        <div className="bg-white rounded-lg border border-border-light p-8 text-center text-text-secondary">
          No questionnaire templates yet. Click &quot;+ Add Question&quot; to create one.
        </div>
      )}
    </div>
  );
