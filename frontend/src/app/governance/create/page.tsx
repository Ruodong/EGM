  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (action === 'next') {
        ...
        toast(t('govCreate.draftSavedContinue'), 'success');
        router.push(`/governance/${result.requestId}?step=2`);
      } else {
        toast(t('govCreate.draftSaved'), 'success');
        router.push(`/governance/${result.requestId}`);
      }
    } catch {
      toast(t('govCreate.failedCreate'), 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageLayout>
      <div className="max-w-2xl mx-auto">
        <h1 className="text-xl font-bold mb-6">{t('govCreate.title')}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div ref={dropdownRef} className="relative">
                      ...
                      {showDropdown && (
                        <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-white border border-border-light rounded-lg shadow-lg max-h-60 overflow-y-auto">
                          {projectLoading && (
                            <div className="px-3 py-2 text-sm text-text-secondary">{t('govCreate.searching')}</div>
                          )}
                          {!projectLoading && projects.length === 0 && projectSearch.trim() && (
                            <div className="px-3 py-2 text-sm text-text-secondary">{t('govCreate.noProjects')}</div>
                          )}
                          {projects.map((p) => (
                            <button
                              key={p.projectId}
                    ...
                    <div>
                      ...
                      <div ref={pmDropdownRef} className="relative">
                        {nonMspo.projectPmItcode ? (
                          ...
                          <Input
                            placeholder={t('govCreate.searchEmployee')}
                            value={pmSearch}
                        ...
                        {showPmDropdown && (
                          <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-white border border-border-light rounded-lg shadow-lg max-h-60 overflow-y-auto">
                            ...
                            {!pmLoading && pmResults.length === 0 && pmSearch.trim() && (
                              <div className="px-3 py-2 text-sm text-text-secondary">{t('govCreate.noEmployees')}</div>
                            )}
                            {pmResults.map((emp) => (
                              <button
                                key={emp.itcode}
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">
                  {t('govCreate.productType')} <span className="text-red-500">*</span>
                </label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium mb-1">
                  {t('govCreate.productEndUser')} <span className="text-red-500">*</span>
                </label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">
                  {t('govCreate.userRegion')} <span className="text-red-500">*</span>
                </label>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button type="default" onClick={() => router.back()} data-testid="cancel-btn">{t('common.cancel')}</Button>
            <Button
              htmlType="submit"
              type="default"
              disabled={loading}
            ...
            >
              {loading && submitActionRef.current === 'save' ? t('common.saving') : t('common.save')}
            </Button>
            <Button
              htmlType="submit"
              type="primary"
            ...
            >
              {loading && submitActionRef.current === 'next' ? t('common.saving') : t('common.next')}
            </Button>
          </div>
        </form>
      </div>
